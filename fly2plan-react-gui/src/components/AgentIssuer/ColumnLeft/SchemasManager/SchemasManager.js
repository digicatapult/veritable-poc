...
export default function SchemasManager({
	origin, onSelectedSchema, selectedSchemaId
}) {
	const maxNumOfSchemas = 2
	const [ dataSchemasCreated, setDataSchemasCreated ] = useState([])
	const [ statusSchemasCreated, errorSchemasCreated, startFetchSchemasCreatedHandler ] = useGetSchemasCreated()
	...
	const createSchemaHandler = (e) => {
		...
		if (statusCreate !== 'error') startFetchHandler(origin, schemaName, setDataFn)
	}
	...
	return (
		<div className="col-md-6">
			...
			<div className="form-group">
				...
				<div className="row">
					<div className="col-md-8 pr-0">
						<select onChange={chooseSchemaHandler}
							value={
								dataSchemasCreated.includes(selectedSchemaId)?selectedSchemaId:''
							}
							name="schema"
					...
					<div className="col-md-4 pl-0">
						<button onClick={createSchemaHandler}
							type="submit"
			...
			<div className={`${errorSchemasCreated?'d-block':'d-none'}`} style={{position: 'fixed', width: '10%', height: '10%', inset: '0px', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 100}}>
