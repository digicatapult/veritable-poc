...
export default function DefinitionsManager(
	{ origin, persona, selectedSchemaId, selectedCredDefId, onSelectedDefinition }
) {
	const [ dataDefinitionsCreated, setDataDefinitionsCreated ] = useState([])
	const [ statusDefinitionsCreated, errorDefinitionsCreated, startFetchHandlerDefinitionsCreated ] =
		useGetCredDefinitionsCreated()
	const [ statusCreate, errorCreate, startFetchHandler ] = usePostCredentialDefinitions()

	useEffect(()=>{
		const setDataFn = (retrievedData) => (setDataDefinitionsCreated(retrievedData))
		startFetchHandlerDefinitionsCreated(origin, selectedSchemaId, setDataFn)
	}, [ origin, selectedSchemaId, selectedCredDefId, startFetchHandlerDefinitionsCreated ])
	...
	return (
		<div className="col-md-6">
			...
			<div className="form-group">
				<label htmlFor="definitions">
					...
					<div className="small">
						List has&nbsp;
						<b>
							{statusDefinitionsCreated!=='fetching' && dataDefinitionsCreated.length}
						</b>
						&nbsp;definition(s)
					</div>
				</label>
				<div className="row">
					<div className="col-md-8 pr-0">
						<select onChange={chooseDefinitionHandler}
							value={
								dataDefinitionsCreated.includes(selectedCredDefId)?selectedCredDefId:''
							}
							name="definition"
					...
					<div className="col-md-4 pl-0">
						<button onClick={createDefinitionHandler}
							type="submit"
			...
			<div className={`${errorDefinitionsCreated?'d-block':'d-none'}`} style={{position: 'fixed', width: '10%', height: '10%', inset: '0px', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 100}}>
				<div className="text-light m-2 p-2">
					<small>Failed to fetch: {errorDefinitionsCreated}</small>
				</div>
			</div>

			<div className={`${errorCreate?'d-block':'d-none'}`} style={{position: 'fixed', width: '10%', height: '10%', inset: '0px', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 100}}>
				<div className="text-light m-2 p-2">
