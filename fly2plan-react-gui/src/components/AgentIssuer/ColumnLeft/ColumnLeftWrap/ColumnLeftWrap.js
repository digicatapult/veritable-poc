...
export default function ColumnLeftWrap({origin, persona}) {
	...
	const [ selectedExpiration, setSelectedExpiration ] = useState(new Date().toISOString().split('T')[0])
	...
	const activatedSubmitHandler = (connectionId) => {
		...
		startFetchHandler(
			origin,
			connectionId, selectedCredDefId, selectedId, selectedName, selectedSurname,
			selectedType, selectedTitle, selectedSubtitle, selectedExpiration,
			setStoreStatusFn, setStoreDataFn
		)
	}
	const insertedIdHandler = (inserted) => { setSelectedId(inserted) }
	const insertedTitleHandler = (inserted) => { setSelectedTitle(inserted) }
	const insertedNameHandler = (inserted) => { setSelectedName(inserted) }
	const insertedSubtitleHandler = (inserted) => { setSelectedSubtitle(inserted) }
	const insertedSurnameHandler = (inserted) => { setSelectedSurname(inserted) }
	const insertedExpirationHandler = (inserted) => { setSelectedExpiration(inserted) }
	const chosenTypeHandler = (chosen) => { setSelectedType(chosen) }
	const toggleSuccessShowHandler = () => { setStatusIssueCred('idle') }
	...
	return (
		<>
				...
				<div className="container py-1">
					<div className="row">
						<div className="col-md-12">
							...
							<h5>
								Issue a set of credentials
								for the holder
							</h5>
							<p className="small">
								Fill in to issue a new
								set of credentials
								(add a new Schema and/or
			...
			<div className={`${errorIssueCred?'d-block':'d-none'}`} style={{position: 'fixed', width: '10%', height: '10%', inset: '0px', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 100}}>
