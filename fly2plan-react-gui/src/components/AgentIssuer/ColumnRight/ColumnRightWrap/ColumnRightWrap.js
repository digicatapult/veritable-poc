...
export default function ColumnRightWrap({origin}) {
	...
	const [ statusRecords, errorRecords, startGetRecordsHandler ] = useGetIssueCredentialRecords()
	useEffect(()=>{
	...
	return (
		<>
			...
			<div className={(errorRecords)?'d-block':'d-none'} style={{position: 'fixed', width: '10%', height: '10%', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 100}}>
				<div className="text-light m-2 p-2"> <small>{errorRecords}</small> </div>
			</div>
