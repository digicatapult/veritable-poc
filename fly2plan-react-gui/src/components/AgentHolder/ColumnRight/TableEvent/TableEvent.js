...
export default function TableEvent({i, j, k, eId, cId, event}) {
	...
	const getPredicate = (byFormatObj) => {
		...
		const predicateName = predicateKeyStr[0].toUpperCase()+predicateKeyStr.substr(1)
	...
	return (
		...
		<>
			...
			<table className="table table-sm mb-0">
				<tbody>
					<tr className="small">
						<td rowSpan={3}>
							<i
								className={`fa fa-lg ${getEventIcon(event.state)}`}
							/>
						</td>
						<td><b>State: </b>{event.state}</td>
						<td><b>Created: </b>{getTime(event.created_at)}</td>
						<td><b>Updated: </b>{getTime(event.updated_at)}</td>
					</tr>
					<tr className="small">
						<td><b>Initiator: </b>{event.initiator}</td>
						<td><b>Predicate: </b>{getPredicate(event.by_format)}</td>
						<td>
							<b>ConnId: </b>
							<a href="/#" onClick={_=>{}} title={cId}
					...
					<tr className="small">
						<td><b>Role: </b>{event.role}</td>
						<td><b>DefId: </b>
							<a href="/#" className="text-primary"
								onClick={_=>{}} title={getDefIdFull(event.by_format)}
							>
			...
			<div className="bg-light">
				<a className="text-primary collapsed"
					href={`#details-${i}-${j}-${k}`}
				...
				<div className="collapse"
					id={`details-${i}-${j}-${k}`}
				>
					<div className="pre-scrollable bg-white"
						style={{height: '150px'}}
					>
						<ReactJson
							src={event}
