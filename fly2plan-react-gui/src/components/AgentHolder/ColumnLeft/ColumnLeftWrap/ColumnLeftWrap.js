...
export default function ColumnLeftWrap({origin}) {
	...
	const [ status, error, startGetCredentialSetsHandler ] = useLoopedGetCredentials()
	useEffect(()=>{
		const setStoreDataFn = (resData) => setDataCredentialSets(resData)
		const intervalIdFetch = startGetCredentialSetsHandler(origin, setStoreDataFn)
	...
	return (
		<>
					...
					<div className="row">
						<div className="col-md-12">
							...
							{dataCredentialSets.length===0 && (
								<p className="small">N/A</p>
							)}

							{
								dataCredentialSets.length>0 && dataCredentialSets.map((i, k) => (
			...
			<div className={(error)?'d-block':'d-none'} style={{position: 'fixed', width: '10%', height: '10%', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 100}}>
				<div className="text-light m-2 p-2"> <small>{error}</small> </div>
			</div>
