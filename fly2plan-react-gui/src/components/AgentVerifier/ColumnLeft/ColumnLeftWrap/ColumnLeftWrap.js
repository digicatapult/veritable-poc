...
import usePostPresentProofRecordsVerifyPresentation from
'../../../../interface/hooks/use-post-present-proof-records-verify-presentation'
...
export default function ColumnLeftWrap({origin}) {
	...
	const [ selectedValidity, setSelectedValidity ] = useState(tomorrow.toISOString().split('T')[0])
	...
	const [ errorSendReq, startFetchHandlerSendReq ] = usePostPresentProofSendRequest()
	const [ statusRecords, errorRecords, startFetchHandlerRecords ] = useGetPresentProofRecords()
	...
	const [ errorVerify, startFetchHandlerVerify ] = usePostPresentProofRecordsVerifyPresentation()
	...
	const submitProofReqHandler = () => {
		startFetchConnIdHandler(origin, function setStoreDataConnCb(connectionId) {
			...
			startFetchHandlerSendReq(
				origin,
				connectionId,
				selectedId, selectedName, selectedSurname, selectedType, selectedValidity,
				setStatusSendReq, function setStoreDataSendReqCb(exId) {
					startFetchHandlerRecords(origin, function setStoreDataRecordsCb(records) {
						let presExIds = records.map(r => r.pres_ex_id)
						presExIds = presExIds.indexOf(exId) > -1 ? presExIds : [ ...presExIds, exId ]
	...
	const submitVerifyHandler = () => {
		const setStoreDataFn = (retData) => { setDataVerify(retData) }
		startFetchHandlerVerify(origin, selectedExchange, setStatusVerify, setStoreDataFn)
		setDataPresExIds(prevExIds => ( prevExIds.filter(e => e !== selectedExchange) ))
	}
	...
	return (
		<>
					...
					<div className="row">
						<div className="col-md-12">
							...
							<p className="small my-2">
								Flight-plan verification form.
								To start again, please
								&nbsp;
							...
							<>
								<VerificationVersionManager isLockedForm={isLockedForm}
							...
							{ selectedVersion === '1' && (
								...
								<SendRequestFormManager onChosenValidity={chosenValidityHandler}
							...
							{ dataPresExIds.length > 0 && (
								...
								<VerifyPresentationManager statusRecords={statusRecords}
							...
							<div className="modal-body text-center">
								<div className="my-3">
									...
									<div className="text-center align-items-center d-flex">
										{ dataVerify?.state==='done' && dataVerify?.verified==='true' && (
											<div className="mx-auto">
												<i className="d-block fa fa-check-square fa-3x text-success mb-3 mt-3" />
												<div className="lead mb-3">Verification: <b>PASS</b></div>
											</div>
										) }
										{ dataVerify?.state==='done' && dataVerify?.verified==='false' && (
											<div className="mx-auto">
												<i className="d-block fa fa-window-close fa-3x text-danger mb-3 mt-3" />
												<div className="lead mb-3">Verification: <b>FAILED</b></div>
											</div>
										) }
									</div>
								</div>
			...
			<div className={`${errorConnId?'d-block':'d-none'}`} style={{position: 'fixed', width: '10%', height: '10%', inset: '0px', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 100}}>
				<div className="text-light m-2 p-2">
					<small>Failed to fetch: {errorConnId}</small>
				</div>
			</div>

			<div className={`${errorSendReq?'d-block':'d-none'}`} style={{position: 'fixed', width: '10%', height: '10%', inset: '0px', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 100}}>
				<div className="text-light m-2 p-2">
					<small>Failed to fetch: {errorSendReq}</small>
				</div>
			</div>

			<div className={`${errorRecords?'d-block':'d-none'}`} style={{position: 'fixed', width: '10%', height: '10%', inset: '0px', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 100}}>
				<div className="text-light m-2 p-2">
					<small>Failed to fetch: {errorRecords}</small>
				</div>
			</div>

			<div className={`${errorVerify?'d-block':'d-none'}`} style={{position: 'fixed', width: '10%', height: '10%', inset: '0px', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 100}}>
				<div className="text-light m-2 p-2">
