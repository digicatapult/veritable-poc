...
export default function ExchangeRecords({dataConnRecordEvents}) {
	return (
		...
		<>
			...
			{ dataConnRecordEvents.length > 0 && dataConnRecordEvents.map( (o, i) => {
				const [ cId, exRecordEvents ] = Object.entries(o)[0]
				return (
					<div key={i} className="text-dark" id={`connection-${i}`}>

						{ dataConnRecordEvents.length > 1 && (
							<h6>Events related to Connection Id <small>{cId}</small>:</h6>
						) }

						{ exRecordEvents.length > 0 && exRecordEvents.map( (p, j) => {
							...
							return (
								<div className="bg-light mb-2"
									...
									id={`wrapper-${i}-${j}`}
								>
									<a className="btn btn-lg btn-primary text-left w-100"
										href={`#details-${i}-${j}`} data-toggle="collapse"
										aria-expanded="true"
									>
									...
									<div className="my-0 p-3 border collapse show"
										id={`details-${i}-${j}`}
									>
										...
										{ events.length > 0 && (
											...
											<div className="accordion" id={`accordion-${i}-${j}`}>
												...
												{ events.map( (event, k) => (
													<div key={k} className="card border border-bottom-1">
