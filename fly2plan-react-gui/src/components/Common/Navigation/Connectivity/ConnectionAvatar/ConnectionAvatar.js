...
export default function ConnectionAvatar({data, onDelete}) {
	...
	*/
	...
	const from  = [
		'created_at',     'updated_at',  'alias',      'invitation_key',
		'connection_id',  'my_did',      'request_id', 'state',          'their_role'
	]
	const to    = [
		'Created@:',      'Updated@:',   'Alias:',     'InvKey:',
		'ConnId:',        'MyDID:',      'ReqId:',     'State:',         'TheirRole:'
	]
	const arrReduceEntries = (conn) => {
		return Object.entries(conn).sort(([ a ], [ b ])=>a.localeCompare(b)).reduce((a, [ k, v ])=>{
			if (from.indexOf(k) > -1) {
	...
	return(
		<>
			...
			{data?.results?.filter(d=>d.state==='active').map((c, k)=>{
				return (
					<li key={k} className="nav-item small">
						<div className="btn-group">
							<a className="p-1 m-1 text-primary bg-light"
										...
										<div className="row">
											<div className="col-md-12">
												<button className="btn btn-primary w-100"
													onClick={()=>{ onDelete(c.connection_id) }}
												>
