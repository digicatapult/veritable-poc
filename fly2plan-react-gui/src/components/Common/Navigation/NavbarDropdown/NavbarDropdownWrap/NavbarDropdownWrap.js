...
export default function NavbarDropdown({status, agent, onSaveOrigin}) {
	const defaultAgName = agent === '2' ? 'User' : (agent === '3' ? 'Verifier' : 'Issuer')
	...
	return (
		<ul className="navbar-nav">
			<li className={`nav-item dropdown ${isVisible ? 'show' : ''}`}>
				...
				{status === 'idle' && (
					<a className="nav-link" data-toggle="dropdown" href="#/" role="button" onClick={clickDropdownHandler} aria-haspopup="true" aria-expanded={isVisible}>
						<NavbarDropdownMultilineBtn line01="Ssi" line02="Port" line03="N/A" />
					</a>
				)}
				{status === 'error' && (
				...
				{status === 'fetching' && (
					<a className="nav-link disabled" href="#/" aria-expanded="false">
						<NavbarDropdownMultilineBtn line01=" " line02=" " line03=" " icon="fa fa-spinner fa-pulse" />
					</a>
				)}
				{status === 'fetched' && (
					<a className="nav-link disabled text-light" data-toggle="dropdown" href="#/" role="button" onClick={e=>e.stopPropagation()} aria-haspopup="true" aria-expanded={isVisible}>
						<NavbarDropdownMultilineBtn line01="Ssi" line02="Port" line03={new URL(origin).port} />
					</a>
				)}

				{status === 'idle' && (
					<div className={`dropdown-menu bg-light p-2 ${isVisible ? 'show' : ''}`} style={{minWidth: '320px'}}>
						<NavbarDropdownForm
