...
export default function usePostCredentialDefinitions() {
	...
	const transformData = (retrievedData) => (retrievedData.credential_definition_id)
	const statusOptions = [ 'idle', 'error', 'fetching', 'fetched' ]
	const [ status, setStatus ] = useState(statusOptions[0])
	const [ error, setError ] = useState(null)
	const onStartFetch = useCallback((fetchOrigin, selectedSchemaId, persona, setStoreData) => {
		const params = {}
		...
		post(fetchOrigin, path, params, body, setStatus, setError, setStoreData, transformData)
	}, [])
