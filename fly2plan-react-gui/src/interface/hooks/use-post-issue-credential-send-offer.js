...
export default function usePostIssueCredentialSendOffer() {
	...
	const onStartFetch = useCallback((
		origin,
		connectionId, credDefId, id, name, surname,
		type, title, subtitle, expiration,
		setStoreStatus, setStoreData
	) => {
		const createBody = (
			connectionId, credDefId, id, name, surname, type, title, subtitle, expiration
		) => {
		...
		post(origin, path, params, body, setStoreStatus, setError, setStoreData, transformData)
	}, [])
