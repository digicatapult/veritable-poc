...
export default function usePostPresentProofSendRequest() {
	...
	const transformData = (retrievedData) => { return retrievedData.pres_ex_id }
	const [ error, setError ] = useState(null)

	const onStartFetch = useCallback((origin, connectionId, id, name, surname, type, validity, setStatus, setStoreData) => {
		const createBody = (connectionId, id, name, surname, type, validity) => {
			...
			const resName = {schema_name:'drone schema', 'attr::name::value':name}
			const resSurname = {schema_name:'drone schema', 'attr::surname::value':surname}
			const resType = {schema_name:'drone schema', 'attr::type::value':type}
			const reqAttrs = [
		...
		post(origin, path, params, body, setStatus, setError, setStoreData, transformData)
	}, [])
